...
void FIFO(Process pr[], int num, int Timepice);          // 先来先服务算法
...
/*  先来先服务算法的实现*/
void FIFO(Process process[], int num, int Timepice)
...
int main()
{
    ...
    cout << endl;
    cout << "选择调度算法：" << endl;
    cout << "1: FIFO  2: 时间片轮换 3: 优先级调度 4: 最短时间优先 " << endl;
    cin >> a;
    const int Size = 30;
    ...
    else //  最短作业算法，先按时间从小到大排序，再调用FIFO算法即可
    {
        sort1(process, num);
        FIFO(process, num, TimePice);
    }
